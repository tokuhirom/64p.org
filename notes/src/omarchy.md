...
Omarchyは「使うファイルを自分でカスタマイズする」前提の作りで、個人設定は`~/.config`、シェルのカスタマイズは`~/.bashrc`、Omarchy自身のファイルは`~/.local/share/omarchy`という置き分けになっている。この構成を活かして`~/.config/hypr/bindings.conf`のような個人設定ファイルだけを選んで複数マシン間で同期する例が、[[mise-dotfiles|miseのdotfiles自動同期]]で紹介されている（`omarchy-update`の前後にチェックポイントを打つ使い方も含む）。
