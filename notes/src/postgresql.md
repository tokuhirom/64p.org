...
商用DBに引けを取らない機能を無償で使える点から、企業の基幹システム・金融システム・政府機関でも採用されている。近年はPostgreSQL互換のマネージドクラウドサービスも増えており、[[neon|Neon]]のようにサーバーレス・従量課金で提供するものもある。また[[pglite|PGlite]]のようにWASMにコンパイルしてブラウザ内で動かすアプローチや、[[pgrust]]のようにAIコーディングエージェントを使ってRustで丸ごと再実装するアプローチも登場している。
