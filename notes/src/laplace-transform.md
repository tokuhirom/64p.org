...
時間領域の関数 \(f(t)\) を複素変数 \(s\) の関数に写す[[integral|積分]]変換。

$$
F(s) = \int_0^\infty f(t)\, e^{-st}\, dt, \qquad s = \sigma + j\omega
$$

一番のご利益は、**時間領域での[[derivative|微分]]・[[integral|積分]]が、s領域では「\(s\) を掛ける／\(s\) で割る」という代数演算になる**こと。[[differential-equation|微分方程式]]が代数方程式に変換されるので、「変換 → 代数的に解く → 逆変換」という手順で常微分方程式が機械的に解ける。初期条件も変換の時点で自然に式へ組み込まれる。
...
[[fourier-transform|フーリエ変換]]が「周波数分解」そのものを目的にするのに対し、こちらは減衰因子を加えた一般化によって発散する信号や過渡応答を扱えるようにし、微分方程式・制御・回路といった「システムを解く」方向に特化している。
