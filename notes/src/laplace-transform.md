...
一番のご利益は、**時間領域での[[derivative|微分]]・[[integral|積分]]が、s領域では「\(s\) を掛ける／\(s\) で割る」という代数演算になる**こと。微分方程式が代数方程式に変換されるので、「変換 → 代数的に解く → 逆変換」という手順で常微分方程式が機械的に解ける。初期条件も変換の時点で自然に式へ組み込まれる。
