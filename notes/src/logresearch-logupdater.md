...
Zennで話題になっていた`logresearch`アカウントによる不審な複数プルリク送信の件が、実は研究論文の評価実験だったと判明した、という指摘。研究のためとはいえ、実プロジェクトのメンテナーに研究であることを明かさずPRを送りつけ、査読前提の「成功事例」として論文に取り込む手法には引っかかりを感じる。2021年のミネソタ大学によるLinuxカーネルへの意図的なバグ入りパッチ送信事件（通称"hypocrite commits"、無断実験が発覚しカーネルからのban騒動になった）と構造が似ており、ソフトウェア工学研究における「実プロジェクトを無断で実験台にする」問題は繰り返されている。
