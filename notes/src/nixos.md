...
Nixpkgsは「卵が先か鶏が先か」問題を解くため、Cコンパイラ・binutils・libc・coreutilsなど約20個のビルド済みバイナリ（bootstrap seed）から出発し、段階的にツールチェインをソースから再ビルドしてシードへの依存を切っていく。この起点のバイナリは監査が難しく、2026年には[[strip-trusting-trust-attack|シードの`strip`を1つ差し替えるだけでディストリビューション全体のバイナリをバックドア入りにできる]]ことが実証されている。
