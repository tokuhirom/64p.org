...
- **2007年: PTW攻撃**（Pyshkin, Tews, Weinmann）— RC4の統計的偏りの解析を改良し、ARPリプレイで能動的にトラフィックを増やす手法と組み合わせて、約4万パケットで104ビット鍵を50%の確率で復元できるまでに高速化。aircrack-ngのデフォルト手法になっており、実時間で数分あれば破れるレベルになった。
