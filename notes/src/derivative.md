...
- **最適化** — 導関数が 0 になる点が極値の候補。機械学習の[[gradient-descent|勾配降下法]]もこの延長
- **[[differential-equation|微分方程式]]** — 未知関数とその導関数の関係式。物理・工学のモデル記述の基本言語。[[laplace-transform|ラプラス変換]]は微分を代数演算に変えてこれを解く道具
- **近似** — 接線による一次近似（\(f(a+h) \approx f(a) + f'(a)h\)）、[[taylor-series|テイラー展開]]への入り口
