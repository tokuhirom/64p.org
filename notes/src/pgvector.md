...
AWS/Azure/GCPなど主要クラウドのマネージドPostgreSQLサービスでも対応しており、既存のBIツールやSQLベースの分析基盤とも相性がよい。PostgreSQLの知見があれば学習コストを抑えて導入できる。[[pglite|PGlite]]のようなWASM版Postgresでも対応拡張機能として使え、ブラウザ内でのローカルベクトル検索が可能になる。
