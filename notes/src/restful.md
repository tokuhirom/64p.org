...
4. **Uniform Interface（統一インターフェース）** — システム全体を単純化する中核制約。以下4つのサブ制約から成る。
   ...
   - 表現を通じたリソースの操作
   - 自己記述的メッセージ
   - **[[hateoas|アプリケーション状態のエンジンとしてのハイパーメディア（HATEOAS）]]**
...
## Fieldingによる[[hateoas|HATEOAS]]の強い主張（2008年のブログ記事）
...
- [[hateoas|HATEOAS]]により、クライアントは事前知識なしにレスポンス内のリンク（アフォーダンス）から次の操作を判断でき、サーバ側の内部実装変更がクライアントに影響しない、長期的な進化可能性を実現できると主張している。
