...
「新しいサービスを作る」を数クリックで完了させる仕組み。リポジトリ作成・CI設定・カタログ登録・初期コードまでをテンプレート化するので、[[golden-path|Golden Path]]を具体的な形にする道具になる。組織のベストプラクティスを「ドキュメントで啓蒙する」のではなく「デフォルトで通る道」に埋め込める。
