...
- 確認プロンプト・スピナー・段階的ウィザードはエージェントの実行を止めてしまう。`--yes`/`--skip-interactive`のような非対話フラグを用意する。
- ただし危険な操作(本番環境の変更等)を無警戒に自動承認させるのは危険。Arcjetの提案する「非対話型確認プロトコル」が参考になる: まず変更内容をJSONで提示し(`confirmation_required`状態)、ユーザー(または上位のオーケストレーター)が承認した上で明示的な`--confirm`フラグを付けて再実行させる。
...
- より深い投資としては、スキーマや`--describe`的なコマンドでランタイムに全コマンド仕様を自己記述させる、[[agent-plugins|Agent Plugins]]のようなパッケージ仕様に沿ってMCPサーバーとして機能を公開する、Markdown形式の軽量な「スキル」ドキュメント(50行程度)を添付する、といった手法がある。
