...
輸送層に[[quic|QUIC]]を使う版のHTTP。TCPの代わりにUDPベースのQUICを使うことで、[[http2|HTTP/2]]までTCP由来で残っていたHead-of-lineブロッキング問題(1つのストリームのパケットロストが同一コネクション上の他の全ストリームを止めてしまう問題)を解消する。IETFにより2022年6月に**RFC 9114**として標準化された。 #protocol #web
