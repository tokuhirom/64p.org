...
ブラウザをペイン内に表示する[[herdr-browser]]というプラグインも存在する。ワークスペースの構成をTOMLで宣言し、git worktreeとしても開ける[[herdr-plus|herdr plus]]もある。本体はマルチプレクサとしての役割に絞り、周辺機能はプラグインに出す設計。

2026年8月にリリースされた[[omarchy|Omarchy]] 4 "Quattro"では、ベースパッケージにherdrが同梱され、従来のtmuxと併存する形になった。エージェントをOSの一級市民として扱うというQuattroの方向性の一部にあたる。

同じく複数エージェントの並列運用を扱うデスクトップアプリとして[[orca|Orca]]があり、代替として比較されることが多い。herdrがターミナル常駐によるセッション永続化を主眼に置くのに対し、Orcaはタスクごとのgit worktree自動生成による環境隔離を主眼に置く。
