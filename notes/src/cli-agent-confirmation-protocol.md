...
実例として[[ansible|Ansible]]には2段階のフラグがある。
...
- 汎用的なパターンとして[[idempotency-key|冪等性キー]]を持たせ、同じキーでの再実行はサーバー側でキャッシュされた結果をそのまま返し、実際の処理を二重に適用しない。
...
CLI側の確認プロトコルは唯一の防衛線ではなく「defense in depth」の一層と位置づけられる。上位層として、[[ai-agent-task-scoped-credentials|エージェントに発行する認証情報自体をタスク単位でスコープ制限する]](カレンダー読み取りエージェントにCRM書き込み権を持たせない等)、セッション満了で自動失効させる、といった認可レベルでの制御と組み合わせるべきという指摘もある。
