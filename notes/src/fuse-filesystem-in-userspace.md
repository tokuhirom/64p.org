...
つまりシステムコール1回につき、カーネル⇔ユーザー空間の往復（コンテキストスイッチ）が発生するため、カーネル内蔵のファイルシステムに比べてオーバーヘッドがある。この往復を実際に`ls`/`cat`で体感する実験を[[fuse-hello-world-experiment|fuse-hello-world実験]]で行った。
