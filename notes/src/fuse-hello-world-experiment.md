...
updated: 2026-08-16 07:53
---
# fuse-hello-world実験

[[fuse-filesystem-in-userspace|FUSE]]が「非rootユーザーでもファイルシステムをマウントできる」「システムコール1回ごとにカーネル⇔ユーザー空間を往復する」という仕組みを手を動かして体感するための実験記録。`libfuse`自体には依存せず、`/dev/fuse`を直接叩く純Rust実装の[[fuser-rust-crate|fuser]]クレート（[[libfuse-api-levels|低レベルAPI]]相当）で、`hello.txt`という1ファイルだけを返す最小のファイルシステムを実装した。

実験は2段階やった。

1. **1回目**: 素朴にマウントして、非rootのままマウント・読み取り・アンマウントができることを確認（成功）
2. **2回目**: マウントの内部動作を[[strace]]で覗いたところ、ptraceの副作用でsetuidが無効化され、今度はマウント自体が失敗した（これはこれで仕組みの理解につながった）
...
## 2回目: マウントの内部動作を[[strace]]で確認

非rootでマウントできる仕組みを確かめるため、`fuser`クレートのソース(`src/mnt/fuse_pure.rs`)を読むと、マウント時は以下の順で処理していることが分かる。
