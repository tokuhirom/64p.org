...
updated: 2026-08-12 13:42
---
# SentencePiece

#llm #nlp

Google 製のオープンソースのテキストトークナイザ／デトークナイザライブラリ。工藤拓（[[mecab|MeCab]]の作者としても知られる）と John Richardson により開発され、2018年に論文が公開された。ニューラルネットベースのテキスト生成システム向けに、語彙サイズを事前に固定してサブワード分割を学習する。サブワードアルゴリズムとしては [[byte-pair-encoding|BPE]] と Unigram 言語モデルの両方を実装している。
