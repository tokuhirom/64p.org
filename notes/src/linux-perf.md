...
## 実装原理: `perf_event_open`システムコール

内部的には計測対象のイベント1つにつき`perf_event_open(2)`を1回呼んでfdを1つ取得し、そのfdから`read()`でカウンタ値を読む。`perf stat`自体を`strace`でトレースして`EACCES`→フォールバックの挙動やハイブリッドCPUでのPMU別扱いを観察した記録を[[perf-event-open-experiment|perf_event_open実験]]に残した。
