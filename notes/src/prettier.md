...
[[eslint|ESLint]]のように何十個ものルールを個別に選ぶ必要がなく、設定項目を最小限にして「いい感じ」に整形してくれるよう設計されている。保存時に自動でルールに沿ってコードを書き換える。
...
従来世代（JS製・単機能）のフォーマッター側の代表。リンター側の代表である[[eslint|ESLint]]と組み合わせて使われることが多い。
