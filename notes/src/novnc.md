...
- [[browser-raw-tcp-restriction|ブラウザは生のTCPソケットを直接扱えない]]ため、WebSocketを介してRFBプロトコルをやり取りする。サーバー側のVNCが素のTCPしか話さない場合は、[[websockify]]（WebSocket⇔TCPのプロキシ／ブリッジ）を挟んでnoVNCと接続する構成が一般的（VNCサーバー自体がWebSocketに対応していれば不要）。
