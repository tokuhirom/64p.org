...
Apache 2ライセンスで公開されているオープンソースのマルチレイヤ仮想スイッチ。仮想マシン環境で動作するソフトウェアスイッチで、L2（データリンク層）でイーサネットフレームを扱いつつ、OpenFlowにも対応している。[[linux-bridge|Linux標準ブリッジ]]より本格的な仮想スイッチとして、OpenStackなどが実際に採用している。 #network #sdn
...
実装形態も複数あり、古典的なカーネルモジュール版のほか、高性能だが設定が複雑なDPDK版、保守性に優れる[[bpf|eBPF]]版、従来ツールとの互換性を保ちながら高速化する[[xdp|AF_XDP]]版が存在する。

## 従来の[[linux-bridge|Linuxブリッジ]]との違い
...
- **[[kubernetes|Kubernetes]]/コンテナ基盤** — [[cni|CNI]]プラグイン経由でPod間ネットワークを構築
