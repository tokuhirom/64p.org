...
Viteはこれまで、開発時にesbuild・本番ビルド時にRollupという2つの異なるバンドラーを使い分ける構成だった。Rolldownはこの二重構成をRust製バンドラー1つに統一することを目指すプロジェクトで、開発ツール(Vite)・バンドラー(Rolldown)・コンパイラ(Oxc)をすべてVoidZeroが同じチームで維持する構成になる。
