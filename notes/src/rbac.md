...
ロールベースアクセス制御。ユーザーに権限を直接付与するのではなく、「ロール」（職務・役割）に権限を束ね、ユーザーにはロールを割り当てるアクセス制御モデル。ユーザーの入退社・異動のたびに個別権限を付け替える代わりにロールの付け替えだけで済むため、管理コストが下がり、最小権限の原則や職務分掌（separation of duties）を実施しやすい。
