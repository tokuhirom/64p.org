...
FTS3の後継として**FTS4**（データ圧縮オプションやcontentlessテーブルなどの改善）、さらに**FTS5**（モジュール設計の刷新、FTS3/4とはSQL構文がやや異なる、`ORDER BY`でのrelevancy（関連度）スコアリングに対応）が追加されている。FTSモジュールは性能改善が継続的に行われているため、新しいバージョンを使う場合はFTS5が推奨される。
