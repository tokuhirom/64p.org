...
Conftestが使うOPA/Regoとは別系統・無関係のプロジェクト。OPAはCNCFプロジェクトでGo実装、RegoはKubernetes admission control・Terraformプラン検証・インフラのコンプライアンスチェックまで幅広く使える汎用ポリシー言語。対してCedarはRust実装で、アプリケーションの認可(principal/action/resource)に用途を絞り、安全性(必ず停止・副作用なし)と実行速度、形式検証性を重視した設計になっている。ベンチマークではCedarがRegoの42〜60倍高速という報告もある。
