...
コンテナを動かすことに特化した軽量Linuxディストリビューション。パッケージマネージャを持たず、アプリケーションはすべてコンテナで動かす前提で、OS自体は自動アップデートされるという、当時としては先進的な設計だった。Red Hat買収後の2020年5月にEOLとなり、公式後継は**Fedora CoreOS**（Red Hat OpenShiftのノードOSであるRHEL CoreOSの上流）。

## 生み出したソフトウェア

会社としてのCoreOSの影響は、OS本体よりも周辺プロジェクトの方が大きく残っている。

- **[[etcd]]** — 分散キーバリューストア。[[kubernetes|Kubernetes]]のプライマリデータストアになり、CNCFへ寄贈された
