...
**個別の漫画・小説・絵本作品についての感想・既読記録は対象外。** `/notes/` は公開サイトのため、具体的な1作品を主題にしたノート（あらすじ・感想・既読状況など）は書かず、`~/obsidian/Private/`（非公開vault）に書くこと。詳細はプロジェクトの `CLAUDE.md` 「個別の漫画・小説・絵本作品についてのノートは非公開vaultへ」を参照。ジャンル・概念そのものを扱うノートは引き続きここに書いてよい。
